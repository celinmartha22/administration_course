...
class TransaksiMasukListPage extends StatefulWidget {
  static const routeName = '/transaksi_masuk_list';
  const TransaksiMasukListPage({Key? key}) : super(key: key);

  @override
  State<TransaksiMasukListPage> createState() => _TransaksiMasukListPageState();
}

class _TransaksiMasukListPageState extends State<TransaksiMasukListPage> {
  late Future<List<TransaksiKasMasuk>> listTransMasuk;
  late List<TransaksiKasMasuk> listTrans;
  ...
  late PaketKursus kursus = PaketKursus(
      idPaketKursus: '',
      namaPaketKursus: '',
      harga: 0,
      berlakuSelama: '',
  ...
  Future<PaketKursus> _futurePaketKursus(String id) {
    return Provider.of<PaketKursusProvider>(context, listen: false)
        .getPaketKursusById(id);
  }

  Future<void> exportToExcel(
      List<TransaksiKasMasuk> transaksi, DateTime? start, DateTime? end) async {
    ...
    if (start != null && end != null) {
      ...
      for (var i = 1; i <= transaksi.length; i += 1) {
        ...
        if (transaksi[i - 1].idJenisKasMasuk != '') {
          kategori = await _futureJenisKas(transaksi[i - 1].idJenisKasMasuk);
        }
        if (transaksi[i - 1].idPaketKursus != '') {
          kursus = await _futurePaketKursus(transaksi[i - 1].idPaketKursus);
        }
        if (date.compareTo(start) >= 0 && date.compareTo(end) <= 0) {
          output.add(transaksi[i - 1]);
          sheet
              .cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: i))
              .value = transaksi[i - 1].idTransaksiKasMasuk;
    ...
    File(path.join(
        '/storage/emulated/0/Download/Laporan Kas Masuk - ${DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now())}.xlsx'))
      ..createSync(recursive: true)
      ..writeAsBytesSync(fileBytes!);

    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Text(
          'Export Laporan Kas Masuk berhasil\nSilahkan buka folder "Download" untuk melihat hasilnya'),
  ...
  Widget build(BuildContext context) {
    return Consumer<TransKasMasukProvider>(
        ...
        if (state.state == ResultStateDb.loading) {
          ...
          return Scaffold(
            appBar: AppBar(
              ...
              elevation: 0,
              title: Text("Transaksi Kas Masuk",
...
List<TransaksiKasMasuk> _searchTransaksi({
  required List<TransaksiKasMasuk> transaksi,
...
class BuildListTransaction extends StatelessWidget {
  ...
  List<TransaksiKasMasuk> listTransaksi;
  DateTime startDate;
  DateTime endDate;
                        ...
                        slidable?.dismiss(
                          ResizeRequest(const Duration(milliseconds: 300), () {
                            ...
                            confirmPopUp(
                                context,
                                "delete_kas_masuk",
                                result[index].idTransaksiKasMasuk,
                                "Hapus",
